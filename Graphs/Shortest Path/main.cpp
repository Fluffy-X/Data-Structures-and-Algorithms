// Shortesy Path Sum and Actual path
#include <iostream>
#include <list>
#include <queue>
#include <vector>
...
void path(int i,int start)
{
    if(shortestpath[i]!=start)
    {
        cout << i << " -> ";
        path(shortestpath[i],start);
    }
    else
        cout << i << " -> " << start ;
}
...
int Graph::dijkstra(int start)
{
    ...
    while(!pq.empty())
    {
        ...
        for(it=adj[v].begin(); it!=adj[v].end();it++)
        {
            if(visited[(*it).first] > (*it).second+d)
            {
                ...
                visited[(*it).first] = (*it).second+d ;
                predecessor[(*it).first] = v ;
            }
        }
    }
    for(i=0;i<V;i++)
    ...
    cout << endl;
    shortestpath.resize(V,0);
    for(i=0;i<V;i++)
    {
        shortestpath[i] = predecessor[i];
    }
    cout << endl << endl ;
    for(i=0;i<V;i++)
    {
