...
{
    MinHeap h(V);
    DIJK.push_back(make_pair(0,0));//DIJK.push_back(make_pair(start,0));
    int MST = 0;
    int i = 0;
    int second;
    int first;
    ...
    visited[0] = 1;//visited[start] = 1;
    for(i = 1; i < V ; i++)//for(i = 0; i < V ; i++)
    {
        h.insertKey(i,INT_MAX);//h.insertKey(i,INT_MAX);
    }

    vector<pair<int,int> >::iterator it;
    for(it = adj[0].begin(); it!=adj[0].end(); it++)//for(it = adj[start].begin(); it!=adj[start].end(); it++)
    {
